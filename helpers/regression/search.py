...
def search_over_observations(X,
    ...
    regression performance by iteratively sub-sampling combinations from the input
    ...
    well the regression model performed on that particular sub-sample.
    ...
    # Cross-validate the regressor on the sub-samples of observations
    for i in to_remove:
        for selection in combinations(range(X.shape[0]), X.shape[0] - i):
            ...
            # Cross-validate the regressor
            cv = cross_validate_regressor(X_try,
                                          y_try,
                                          model,
                                          metrics=metrics,
                                          num_folds=num_folds,
